# Manvaditya Singh — Portfolio

A personal portfolio website showcasing my work as a **Video Editor, Motion Designer, and Visual Storyteller**.

Live Demo - https://manvaditya-portfolio.vercel.app/

## ✦ About

I'm Manvaditya Singh, a creative editor focused on turning ideas, footage, and stories into engaging visual experiences.

This portfolio showcases my selected work, creative skills, tools, experience, and approach to visual storytelling.

## ✦ What I Do

- Video Editing
- Motion Graphics
- Reels & Short-form Content
- Cinematic Edits
- Visual Storytelling
- Graphic Design
- Content Editing

## ✦ Tools

- Adobe Premiere Pro
- Adobe After Effects
- CapCut

## ✦ Website

The portfolio includes:

- Hero / Introduction
- About
- Work Experience
- Tools & Skills
- Selected Work
- Featured Project
- Contact Section
- Interactive animations and effects
- Video-based project showcase

## ✦ Tech Stack

- HTML5
- CSS3
- JavaScript
- SVG
- Responsive Web Design

## ✦ Design

The website follows a minimal editorial-inspired visual style with:

- Black & off-white color palette
- Cinematic visual elements
- Editorial typography
- Motion and hover interactions
- Responsive layouts
- Video-focused project presentation

## ✦ Author

**Manvaditya Singh**

Video Editor · Motion Designer · Visual Storyteller

---

*Every frame has a story.*
